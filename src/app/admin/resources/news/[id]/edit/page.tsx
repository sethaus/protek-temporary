...
// import { newsData, type NewsItem } from '@/data/news-events'
...
export default function EditNewsPage() {
  ...
  useEffect(() => {
    // Load existing news data from API
    const fetchNews = async () => {
      try {
        ...
        if (result.success) {
          const existingNews = result.data
          ...
          setForm({
            ...
            tags: existingNews.tags?.length > 0 ? existingNews.tags : [''],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Hata: Haber güncellenirken bir sorun oluştu.')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">Haber bilgileri yükleniyor...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600">{news.title} - ID: {news.id}</p>
          </div>
        </div>
      </div>
